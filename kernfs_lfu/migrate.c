...
lru_node_t *g_lfu_hash[g_n_devices + 1];
struct lfu g_lfu[g_n_devices + 1];
...
static int migrate_threshold[g_n_devices + 1] = {0, 50, 95, 100, 0};
...
int update_lfu_list_from_digest(uint8_t dev, lru_key_t k, lru_val_t v)
{
	struct inode *inode;
	lru_node_t *node;
	lfu_node_t *freq, *next_freq;

	HASH_FIND(hh, g_lfu_hash[dev], &k, sizeof(lru_key_t), node);

	if (node) {
		freq = node->parent;	
		next_freq = freq->next;
		if((!next_freq->next) || 
		   next_freq->count != freq->count + 1) {
			next_freq = get_new_lfu_node(freq->count + 1, freq, next_freq);
		}
		// move data node to next frequency list
		list_del_init(&node->list);
		list_add(&node->list, &(next_freq->item_head.list));
		node->parent = next_freq;
		freq->n--;
		next_freq->n++;
		// delete frequency list if empty
		if(freq->n == 0) {
			delete_lfu_node(freq);
		}	
	} else {
		// case 1: list of frequency 1 already exists
		// case 2: doesn't exist yet
		freq = (g_lfu[dev].lfu_head)->next;
		if (!freq || freq->count != 1) {
			freq = get_new_lfu_node(1, g_lfu[dev].lfu_head, freq);
		}	
		...
		HASH_ADD(hh, g_lfu_hash[dev], key, sizeof(lru_key_t), node);
		node->sync = 0;
		list_add(&node->list, &(freq->item_head));
...
int update_slru_list_from_lru(uint8_t dev, lru_key_t k)
{
	struct inode *inode;
	lru_node_t *node;
...
again:
	...
	//chunkai: should use reverse; otherwise, the LRU order is wrong?
	//list_for_each_entry_safe(l, tmp, &migrate_success_list, list) {
	list_for_each_entry_safe_reverse(l, tmp, &migrate_success_list, list) {
		handle_t handle;
...
do_force_migration:
	...
	lfu_node_t* freq = (g_lfu[dev].lfu_head)->next;
