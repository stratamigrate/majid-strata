...
void show_storage_stats(void)
{
	...
	printf("HDD - %.2f %% used (%.2f / %.2f GB) - # of used blocks %lu\n", 
		...
		mlfs_info("--- lfu_list count %lu, %lu, %lu\n",
			  g_lfu[g_root_dev].n, g_lfu[g_ssd_dev].n, g_lfu[g_hdd_dev].n);
}
		...
		offset_t offset, uint32_t length, addr_t blknr)
{
	...
	if ((length < g_block_size_bytes) || offset_in_block != 0) {
		...
		update_lfu_list_from_digest(to_dev, k, v);
#endif
	...
	while (nr_digested_blocks < nr_blocks) {
		...
		//mlfs_info("Update LFU list from %u to %u\n", from_dev, to_dev); 
#endif
...
static void update_lru_for_read(void) 
{
	...
	list_for_each_entry_safe_reverse(l, tmp, &lru_heads[g_log_dev], list) {
		...
		update_lfu_list_from_digest(dev, l->key, l->val);
	}
}
#endif
...
*/
void init_device_lfu_list(void)
{
	int i;

	for (i = 1; i < g_n_devices + 1; i++) {
		memset(&g_lfu[i], 0, sizeof(struct lfu));
		g_lfu[i].lfu_head.next = NULL;
		g_lfu[i].lfu_head.prev = NULL;
		g_lfu_hash[i] = NULL;
	}
