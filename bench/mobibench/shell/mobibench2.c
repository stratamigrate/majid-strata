...
typedef enum
{
  ...
  MODE_READ,
  MODE_RND_READ,
  MODE_WRITE_READ, // Sequential write and then read
  MODE_RND_WRITE_READ,
  MODE_ZIPF_WRITE_READ,
} file_test_mode_t;
...
int write_ratio = 50; // write ratio for random read & write mode
...
void print_time(struct timeval T1, struct timeval T2)
{
	...
	if(db_test_enable == 0)
	{
		...
		printf("[TIME] :%8ld sec %06ldus. \t%.0f B/sec, \t%.2f KB/sec, \t%.2f MB/sec.", 
				sec,usec, rate, rate/1024, rate/1024/1024);
		if(Latency_state==1) 
			fprintf(Latency_fp,"[TIME] :%8ld sec %06ldus. \t%.0f B/sec, \t%.2f KB/sec, \t%.2f MB/sec.",sec, usec, rate, rate/1024, rate/1024/1024);

		if(g_access == MODE_RND_WRITE || g_access == MODE_RND_READ)
		{
			printf(" %.2f IOPS(%dKB) ", rate/1024/reclen, (int)reclen);
			if(Latency_state==1) 
				fprintf(Latency_fp," %.2f IOPS(%dKB) ", 
					rate/1024/reclen, (int)reclen);
		}
		printf("\n");
#ifdef ANDROID_APP
		if(g_access == MODE_RND_WRITE || g_access == MODE_RND_READ)
...
int zipf(double alpha, int n)
{
  ...
  do
  {
    z = genrand64_int64()/(double)(ULLONG_MAX);
  }
  while ((z == 0) || (z == 1));
...
int thread_main(void* arg)
{
	...
	recop = (char *)malloc(sizeof(*recop)*numrecs64);
	...
	if(tmp_recnum == NULL || recnum == NULL){
		...
		long long n_other_entries = numrecs64 - n_overlap_entries;
  		...
  		if (g_access == MODE_ZIPF_WRITE_READ) {
			for(i = 0; i < numrecs64; i++) {
				recnum[i] = zipf(1, numrecs64-1);
			}
		} else {	
		...
		for(i = 0; i < numrecs64; i++) {
			...
			big_rand=genrand64_int64();
			if((int)(big_rand%100) < write_ratio) {
				recop[i] = OP_WRITE;
			} else {
				recop[i] = OP_READ;
			}	
...
//			printf("Unlink %s failed\n", filename);
	...
	if(g_access == MODE_WRITE || g_access == MODE_RND_WRITE || g_access == MODE_WRITE_READ)
	{
		printf("Do writes\n");
		for(i=0; i<numrecs64; i++)
		{
	...
	if(g_access == MODE_READ || g_access == MODE_RND_READ || g_access == MODE_WRITE_READ)
	{
		printf("Do reads \n");
		for(i=0; i<numrecs64; i++)
		{
	...
	if (g_access == MODE_RND_WRITE_READ || g_access == MODE_ZIPF_WRITE_READ) {

		printf("Do random reads and writes \n");
		for(i=0; i<numrecs64; i++)
		{
			offset = (long long)recnum[i]*1024*reclen;	 	 
...
int main( int argc, char **argv)
{
	...
	while((cret = getopt(argc,argv,"p:f:r:a:y:t:d:n:j:s:g:i:hqL:k:v:w:")) != EOF){
		switch(cret){
			case 'p':
	...
	{
		// chunkai: mkdir instead of goto out
		/*
		DIR* dir;
