...
void show_storage_stats(void)
{
	...
	printf("HDD - %.2f %% used (%.2f / %.2f GB) - # of used blocks %lu\n", 
		...
		printf("--- extra lru_list count ");
		uint8_t i;
		for (i=0; i<EXTRA_NVM_LRU_LIST; i++) {
			printf("%lu, ", g_lru_extra[i].n);
		}
		printf("\n");
#endif
		...
		addr_t *loghdr_to_digest, int *rotated)
{
	...
	mlfs_info("Enter digest_logs, loghdr_addr: %lu, n_drs: %u\n", 
			*loghdr_to_digest, n_hdrs);
	...
	for (i = 0 ; i < n_hdrs; i++) {
		...
		if (loghdr_meta->loghdr->inuse != LH_COMMIT_MAGIC) {
			mlfs_info("log header addr %lu, inuse %04x\n", 
					*loghdr_to_digest, loghdr_meta->loghdr->inuse);
...
#ifdef LRU_REQUEST
static void update_lru_for_read(void)
{
        lru_node_t *l, *tmp;
        list_for_each_entry_safe_reverse(l, tmp, &lru_heads[g_log_dev], list) {
                uint8_t dev = l->key.dev;

		//mlfs_info("Update LRU read to dev: %u, blockno: %lu\n", dev, l->key.block);
                update_slru_list_from_digest(dev, l->key, l->val);
        }
}
#endif
...
#if EXTRA_NVM_LRU_LIST
	for (i=0; i<EXTRA_NVM_LRU_LIST; i++) {
		memset(&g_lru_extra[i], 0, sizeof(struct lru));
		INIT_LIST_HEAD(&g_lru_extra[i].lru_head);
		g_lru_hash_extra[i] = NULL;
	}	
#endif
	return;
}
