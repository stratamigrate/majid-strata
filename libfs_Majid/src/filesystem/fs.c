...
struct lru g_fcache_head_region1;
struct lru g_fcache_head_region2;
struct lru g_fcache_head_region3;
struct lru g_fcache_head_region4;
struct lru g_fcache_head_region5;
struct lru g_fcache_head_region6;
struct lru g_fcache_head_region7;
...
uint64_t g_fcache_accesses = 0;
uint64_t g_fcache_misses = 0;
uint64_t g_root_dev_accesses = 0;
uint64_t g_root_dev_misses = 0;
...
void shutdown_fs(void)
{
	...
	printf("file data cache miss rate: %lf \n", g_fcache_misses/(double)g_fcache_accesses);
	printf("NVM shared area miss rate: %lf \n", g_root_dev_misses/(double)g_root_dev_accesses);
...
static void cache_init(void)
{
	...
	INIT_LIST_HEAD(&g_fcache_head_region1.lru_head);
	INIT_LIST_HEAD(&g_fcache_head_region2.lru_head);
	INIT_LIST_HEAD(&g_fcache_head_region3.lru_head);
	INIT_LIST_HEAD(&g_fcache_head_region4.lru_head);
	INIT_LIST_HEAD(&g_fcache_head_region5.lru_head);
	INIT_LIST_HEAD(&g_fcache_head_region6.lru_head);
	INIT_LIST_HEAD(&g_fcache_head_region7.lru_head);
		...
		offset_t off, uint8_t *data)
{
	...
	//Majid: not all reads eligible to promte, if it is hot, it will promote regardless
...
#ifdef LRU_REQUEST
		// update lru list in kernfs
		while(make_lru_request_async(g_root_dev, bmap_req.block_no) != -EBUSY);
#endif
	} 
