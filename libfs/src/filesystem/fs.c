		...
		offset_t off, uint8_t *data)
{
	...
	//Majid: not all newly added reads must promote, if it is hot, it will promote regardless
	//printf("no more promotion on add\n");
	//list_move_tail(&_fcache_block->l, &g_fcache_head.lru_head); 
...
*/
	...
	for(int i = 0 ; i < MAXSHIFT; i++)
		list_rotate_left(&g_fcache_head.lru_head);
...
int do_unaligned_read(struct inode *ip, uint8_t *dst, offset_t off, uint32_t io_size)
{
	...
	if (_fcache_block) {
		HitMissCounter++;
		if(HitMissCounter == MAXCNT)
			HitMissCounter = MAXCNT-1;
		// read cache hit
		if (_fcache_block->is_data_cached) {
			memmove(dst, _fcache_block->data + (off - off_aligned), io_size);
			//Majid: not promote
			//printf("cache hit in unalinged\n");
			list_move(&_fcache_block->l, &g_fcache_head.lru_head);
	...
	if (bmap_req.dev == g_root_dev) {
		...
		lru_key_t k = {
                  .dev = g_root_dev,
                  .block = bmap_req.block_no,
                };
                lru_val_t v = {
                  .inum = ip->inum,
                  .lblock = bmap_req.start_offset,
                };
		slru_upsert(ip, &lru_heads[g_log_dev], k, v);
#endif
	} 
