...
{
	// chunkai: this will hang if kernfs crashes
	return;
...
static void commit_log(void)
{
	...
	if (loghdr->n > 0) {
		...
		//mlfs_info("[commit] loghdr %lu inuse %04x\n",
		//		loghdr_meta->log_blocks, loghdr->inuse);
...
int make_lru_request_async(uint32_t dev, addr_t blkno)
{
	char cmd_buf[MAX_CMD_BUF] = {0};
	int ret = 0;
	uint32_t count = 1, dummy = 0;

	sprintf(cmd_buf, "|lru |%d|%u|%lu|%u|", dev, count, blkno, dummy);

	if (!g_fs_log->digesting && atomic_load(&g_log_sb->n_digest) > 0) {
		set_digesting();
		mlfs_debug("Send lru command: %s\n", cmd_buf);
		ret = write(g_fs_log->digest_fd[1], cmd_buf, MAX_CMD_BUF);
		return 0;
	} else
		return -EBUSY;
...
int make_lru_request_sync(uint32_t dev, addr_t blkno)
{
	char cmd[MAX_CMD_BUF] = {0};
	int ret = 0;
	uint32_t count = 1, dummy = 0;

	socklen_t len = sizeof(struct sockaddr_un);
	sprintf(cmd, "|lru |%d|%u|%lu|%u|", dev, count, blkno, dummy);
...
static void cleanup_lru_list(void)
{
	lru_node_t *node, *tmp;
	int i = 0;
		...
		for (i = 0; i < n; i++) {
			...
			if (_fd == g_fs_log->digest_fd[0]) {
				...
				if (cmd_buf[1] == 'd') {
					...
					char cmd_header[10];
					uint32_t dev, count, dummy;
					addr_t blkno;

					sscanf(cmd_buf, "|lru |%d|%u|%lu|%u|", &dev, &count, &blkno, &dummy);

					make_lru_request_sync(dev, blkno);
				}	
#endif
