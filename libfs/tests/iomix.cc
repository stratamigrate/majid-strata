...
typedef enum {ZIPF_MIX, RAND_MIX} test_t;
...
class io_bench : public CThread 
{
	public:
		io_bench(int _id, unsigned long _file_size_bytes, unsigned int _io_size,
			 float _write_ratio, test_t _test_type);
...
io_bench::io_bench(int _id, unsigned long _file_size_bytes, 
		unsigned int _io_size, float _write_ratio, test_t _test_type)
	: id(_id), 
...
void io_bench::prepare(void)
{
	...
	// Generate IO list
	uint64_t num_io = file_size_bytes / io_size;
	if (test_type == RAND_MIX) {
	...
	for (uint64_t i = 0; i < num_io; i++) {
		if (dist(mt) < write_ratio) {
			op_list.push_back(OP_WRITE);
		} else {
			op_list.push_back(OP_READ);
		}	
	}	
	/*
	for(auto it : io_list)
...
void io_bench::do_read_write(void)
{
	if (per_thread_stats) {
...
test_t io_bench::get_test_type(char *test_type)
{
	 ...
	 */
	if (!strcmp(test_type, "rand")){
		return RAND_MIX;
	}
	else if (!strcmp(test_type, "zipf")) {
		return ZIPF_MIX;
	}
	else { 
		show_usage("iobench");
...
{
	std::cerr << "usage: " << prog
		<< " [-d <directory>] <rand/zipf>"
		<< " <size: X{G,M,K,P}, eg: 100M> <IO size, e.g.: 4K> <# of thread> <write_ratio>"
      << endl;
}
...
int main(int argc, char *argv[])
{
	...
	write_ratio = std::stof(argv[5]);
	if (write_ratio > 1.0 || write_ratio < 0.0) {
		std::cerr << "Write ratio must in [0, 1] but you specify: " << write_ratio << std::endl;
		exit(-1);
	}	
